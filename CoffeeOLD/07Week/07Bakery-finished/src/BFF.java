...
public class BFF
{
    ...
    //TODO: error checking for incorrect card type
    public Card_Issuer getCardIssuer(){
         String card = inputLine("Who is the credit card issuer? Type: " + Arrays.asList(Card_Issuer.values()));
        Card_Issuer issuer = Card_Issuer.valueOf(card.toUpperCase());
        return issuer;
    }
    public CreditCard inputCreditCardInfo(){
        String name = inputLine("What is the full name on credit card?");
        Card_Issuer issuer = getCardIssuer();
        String cardNum = inputLine("What is the credit card number?");
        int cvv = inputInt("What is the cvv?");
        
        int month = inputInt("What is the exp month?", 1, 12);
        int year = inputInt("What is the exp year?", 2021, 2030);
        
        return new CreditCard(cardNum, name, issuer, cvv, month, year);
    }

	public Size inputSize() {
		print("All cupcakes come in these sizes: " + Arrays.asList(Size.values()));
		Size size = null;
		boolean good = false;
		while(!good) {
			String word = this.inputWord("please enter size");
