...
public class Bakery
{
	...
	private void loadLoyaltyMembers(){
		...
		ArrayList<String> data = FileReader.readFile("bin/myMembers.txt");
		// NOTE: spot 0 in the list is the HEADER to the file
		for(int i = 1; i < data.size(); i++) {
			String line = data.get(i);
			// one line of data represents a credit card based on the header
			Scanner parser = new Scanner(line); //give Scanner the LINE of data
			parser.useDelimiter("\t"); // tab separated is sometimes easier
			String cardNum = parser.next();
			String name = parser.next();
			String issue = parser.next();
			...
			// goes to the ENUM - turn a string into the ENUM type
			// .values in enums -> give the ARRAY of all the options
			// valueOf method will take the enum name as a string and give back the enum value
			Issuer issuer = Issuer.valueOf(issue.toUpperCase());
	...
	//	int left = num;
		Cupcake[][] tray = new Cupcake[4][3]; // 2d array for a cupcake tin 
		for(int i = 0; i < num; i++){
