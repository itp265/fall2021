...
public class Bakery
{
	...
	private void loadLoyaltyMembers(){
		...
		loyaltyClubCards.add(new CreditCard("Kendra W", Card_Issuer.VISA));
		// could get data from BFF.... or be lazy and hard code to test this out
		loyaltyClubCards.add(new CreditCard("4444111122223333", "Gabi M",Card_Issuer.VISA, 711,  10000, 500, 5, 2022, .12));
		loyaltyClubCards.add(new CreditCard("Sean V", Card_Issuer.AMEX));
		...
		for(int i = 1; i < dataInFile.size(); i++) { // start at line 1 instead of 0
			...
			Card_Issuer issuer = Card_Issuer.matchCardType(cardType); // code will crash if not a valid card-type
