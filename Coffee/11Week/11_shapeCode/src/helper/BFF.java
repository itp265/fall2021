...
import people.CourseMenu;
import people.Person;
import people.Student;
/**
 * BFF is a utility class for ITP 265 that helps provide a friendly way to read input from
 * a user and verify that the input is correct. 
 *
 * DO NOT SHARE THIS WITH OTHER STUDENTS, IT IS A GOOD EXAMPLE FOR EVERYONE
	* TO CODE AND PRACTICE WITH.
 * 
...
public class BFF {
	...
	public Student inputStudent() {
		// get name
		String name = this.inputLine("Enter student name:");
		// get birthday
		LocalDate bday = this.getBirthday();
		// get gpa
		double gpa = this.inputDouble("Enter gpa:");
		//get major
		String major = this.inputLine("Enter major:");
		return new Student(name, bday, gpa, major); //mutable objecy
	}

	public Person inputPerson() {
		// get name
		String name = this.inputLine("Enter name of person:");
		// get birthday
		LocalDate bday = this.getBirthday();
	...
	public CourseMenu getCourseMenuOption(){
		String prompt = "*****\tMenu Options for 265 Program\t*****";

		for(CourseMenu m : CourseMenu.values()){ //array from the enum
			prompt += "\n" + (m.ordinal() + 1) + ": " + m.getDisplayString();
		}
		prompt+="\n**********************************************\n";

		int choice = this.inputInt(prompt, 1, CourseMenu.values().length);
		return CourseMenu.values()[choice-1];
	}
}